  ...
  <div class="rule">
    <div class="pageHeader">
		<div class="pageBread">
			...
			<BreadcrumbItem>精选文章</BreadcrumbItem>
			</Breadcrumb>
		</div>
...
export default {
    ...
    data () {
        return {
            pageShow:true,
            formValidate:{
                courseId:this.$store.state.news.courseIdStar,
                type:this.$store.state.news.typeStar,
                status:this.$store.state.news.statusStar
            },
            // 分页
           pageData:{
                //分页数
    ...
    watch:{
        "formValidate.type":function(val){
            this.$store.commit("typeSearchStar",val)
        },
        "formValidate.courseId":function(val){
            this.$store.commit("courseIdSearchStar",val)
        },
        "formValidate.status":function(val){
            this.$store.commit("statusSearchStar",val)
        },
    },
    created(){
    ...
    methods:{
    	list(){
    		let _this=this;
    		let data={
                page:_this.$store.state.news.pageStar.pageCurrent,
                pageSize:_this.$store.state.news.pageStar.pagesize,
                star:1
    		}
            if (this.$store.state.news.courseIdStar) {
                data.courseId=this.formValidate.courseId
            }
            if (this.$store.state.news.typeStar) {
                data.type=this.formValidate.type
            }
            if (this.$store.state.news.statusStar==0||this.$store.state.news.statusStar==1) {
                data.status=this.formValidate.status
            }
            // if (this.$store.state.news.statusStar) {
            //     data.status=this.formValidate.status
            // }
    		this.$api.get("/admin/api/news/list",data,result=>{
    			if (result.code==200) {
                    ...
                    _this.$store.dispatch('pageSearchStar',_this.pageData);
                    _this.pageShow =  true
    			}else{
        ...
        pageComponentDate (e) {
            ...
            this.$store.dispatch('pageSearchStar',this.pageData);
            this.list()
        },
        exportEditData(e){
