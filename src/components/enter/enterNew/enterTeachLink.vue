<template lang="html">
    <div class="">
        ...
        <div class="page-component-card">
            <Card style="width:100%;">
                <p slot="title">
                    <span>链接 <i>注：此种方式不能录制题干分离的题型，例如：现代文阅读，完型填空等，此种形式的题型</i></span>
                </p>
                <Form ref="formLink" :model="formLink" label-position="right">
                    <FormItem prop="linkurl">
                        <Input type="textarea" v-model="formLink.linkurl" placeholder="请输入链接"></Input>
                    </FormItem>
                </Form>
            </Card>
        </div>
        <div style="text-align:center;margin-bottom:15px;">
            <Button type="primary" size="large" @click="sure('formLink')">提交</Button>
        </div>
...
export default {
    ...
    beforeRouteEnter(to,from,next){
        next((vm)=>{
            if (to.path=='/enter/teach/link') {
                vm.$store.commit('itemNameEnter','n1')
            }
        })
    },
    created(){
            ...
            this.$refs[name].validate((valid) => {
                if(valid){
                    var data={
                        ...
                        webUrl:this.formLink.linkurl,
                    };
                    if(this.$route.query.tid){
                        data.textbookId=this.$route.query.tid
                    }
