...
export default {
    ...
    mounted(){
        ...
        this.exportDatas.scope=this.importScope;
    },
    methods:{
        // 修改小题题干
        changeValue(val,content){
            if(this.exportDatas.id==0){
                this.exportDatas.id=val.id;
                if(val.uid){
                    this.exportDatas.uid=val.uid;
                }
                this.exportDatas.jsonContent=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
            }else if (this.exportDatas.id==val.id) {
                this.exportDatas.jsonContent=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
            }else {
        ...
        changeAnalysis(val,value){
            if(this.exportDatas.id==0){
                ...
                this.exportDatas.stringAnalysis=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
            }else if (this.exportDatas.id==val.id) {
                this.exportDatas.stringAnalysis=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
        ...
        changeSelectionKey(val,value){
            if(this.exportDatas.id==0){
                ...
                this.exportDatas.stringSelectionKey=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
            }else if (this.exportDatas.id==val.id) {
                this.exportDatas.stringSelectionKey=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
        ...
        changeAnswer(val,value){
            if(this.exportDatas.id==0){
                ...
                this.exportDatas.stringAnswer=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
            }else if (this.exportDatas.id==val.id) {
                this.exportDatas.stringAnswer=event.target.value || 1;
                this.$emit("exportData",this.exportDatas)
        ...
        changeItem(val,key,value){
            if(this.exportDatas.id==0){
                this.exportDatas.jsonValue=JSON.parse(value);
                this.exportDatas.jsonValue[key]=event.target.value;
                this.exportDatas.id=val.id;
                ...
                if (Object.keys(this.exportDatas.jsonValue).length==0) {
                    this.exportDatas.jsonValue=JSON.parse(value);
                }
                this.exportDatas.jsonValue[key]=event.target.value;
                this.$emit("exportData",this.exportDatas)
        ...
        sure(id){
            let _this = this
            if(_this.exportDatas.id!=0 && _this.exportDatas.id!=id){
                this.$Message.error("请先提交修改选项,再修改其他选项!")
            }else if(_this.exportDatas.id==0){
